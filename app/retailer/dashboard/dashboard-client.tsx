...
import type { DashboardRecentOrder } from "@/actions/retailer/get-dashboard-data";
...
interface DashboardClientProps {
  initialHotDeals: RetailerProduct[];
  initialRecentOrders: DashboardRecentOrder[];
  initialShippingOrders: OrderDetail[];
}
...
export default function DashboardClient({
  ...
  // 초기 데이터를 state로 관리
  const [hotDeals] = useState<RetailerProduct[]>(initialHotDeals);
  const [recentOrders] = useState<DashboardRecentOrder[]>(initialRecentOrders);
  const [shippingOrders] = useState<OrderDetail[]>(initialShippingOrders);
