...
- [X] 베스트 랭킹 페이지 (R.SEARCH.06)
  ...
  - [X] 판매량 기준 정렬 구현 (완료된 주문의 판매량 집계)
  - [X] 베스트 리스트 아이템 상품 제목 클릭 시 상품 상세 페이지 이동 기능
...
- [X] 결제 승인 API 연동 (Payment_API.md 참조)
  - [X] 결제 성공 페이지 생성 (`app/retailer/checkout/success/page.tsx`)
    - [X] URL 쿼리 파라미터에서 `paymentKey`, `orderId`, `amount` 추출
    - [X] 결제 승인 서버 액션 호출 (`actions/retailer/confirm-payment.ts`)
    - [X] 성공 시 주문 내역 페이지로 자동 리다이렉트 (3초 후)
    - [X] 실패 시 에러 메시지 표시 및 재시도 버튼 제공
    - [X] 다크모드 대응 및 반응형 디자인
  - [X] 결제 승인 서버 액션 생성 (`actions/retailer/confirm-payment.ts`)
  ...
  - [X] 결제 요청 페이지 수정 (`app/retailer/checkout/checkout-client.tsx`)
    ...
    - [X] `orderId`, `amount` 파라미터 전달 확인
    - [X] 토스페이먼츠 리다이렉트 시 자동으로 쿼리 파라미터 추가됨
  - [X] 주문 생성 로직 확인
    - [X] 결제 요청 전 주문 ID 생성 (`createPayment` 호출)
    - [X] 주문 생성 실패 시 결제 요청 차단 확인
    - [X] 생성된 `orderId`를 결제 요청에 전달 확인
    - [X] 주문 상태: `'pending'` (결제 대기) 명시
    - [X] 상세 로깅 추가 (주문 생성 시점, 전달 과정)
...
### 🛠 최근 수정 로그
- 2025-12-26: 재주문 장바구니 저장 로직(DB 기준) 통일
- 2025-12-26: 주문 목록 재주문 버튼 - cart 옵션(리테일러ID/클라이언트) 준비 전 비활성화 + 주문목록에서 옵션 조회 1회로 통일
  - 주문 내역의 재주문 버튼이 메모리만 업데이트하던 문제를 수정
  - `useCartOptions()`의 `retailerId/supabaseClient`를 사용해 `cart_items`에 UPSERT 후 장바구니로 이동
- 2025-12-26: 주문 상세 재구매 버튼 - 상품 상세 이동 제거 + 장바구니 담기 후 장바구니 페이지로 이동
- 2025-01-XX: 베스트 페이지 판매량 기준 정렬 및 UX 개선
  - 베스트 페이지 순위 로직 개선
    - `getAllBestRetailerProducts` 함수를 판매량 기준으로 변경
    - 완료된 주문(`completed`, `shipped`, `confirmed`)의 판매량 집계
    - 판매량이 많은 상품 순서대로 정렬하여 진정한 베스트 상품 표시
    - 최근 등록 순서가 아닌 실제 판매 성과 기준으로 순위 결정
  - 베스트 리스트 아이템 UX 개선
    - 4-10위 상품 리스트에서 상품 제목 클릭 시 상품 상세 페이지로 이동 기능 추가
    - `BestListItem` 컴포넌트의 `<h3>` 요소에 `onClick` 이벤트 및 `cursor-pointer` 클래스 추가
    - 상품 이미지 클릭과 동일한 동작으로 상세 페이지 이동
- 2025-01-XX: 결제 승인 API 연동 구현 (Payment_API.md 참조)
  - 결제 성공 페이지 구현
    - 결제 성공 후 자동으로 결제 승인 API 호출
    - 성공/실패 상태별 UI 표시 (로딩, 성공, 에러)
    - 성공 시 3초 후 주문 내역 페이지로 자동 리다이렉트
    - 실패 시 재시도 버튼 제공
  - 결제 승인 서버 액션 생성
    - `/api/payments/confirm` API 호출 래퍼 함수
    - 필수 파라미터 검증 및 에러 처리
  - 결제 요청 페이지 수정
    - `successUrl` 경로 수정 (`/retailer/checkout/success`)
    - `orderId`, `amount` 파라미터 전달 확인
  - 주문 생성 로직 확인
    - 결제 요청 전 주문 ID 생성 확인
    - 주문 생성 실패 시 결제 요청 차단 확인
    - 주문 ID 전달 과정 로깅 추가
- 2025-01-XX: 주문 내역 페이지 재주문 및 주문 취소 기능 구현
  - 재주문 기능 구현
    - 주문 목록에서 재주문 버튼 클릭 시 해당 주문의 상품을 장바구니에 추가
    - 상품 정보를 조회하여 최신 가격 및 배송비 반영
    - 장바구니 페이지로 자동 이동
  - 주문 취소 기능 구현
    - 주문 취소 Server Action 생성 (`actions/retailer/cancel-order.ts`)
    - 준비 중(pending, confirmed) 상태의 주문만 취소 가능
    - 주문 취소 확인 모달 생성 (`components/retailer/cancel-order-modal.tsx`)
    - 주문 취소 시 재고 자동 복구
    - 주문 취소 후 페이지 새로고침으로 상태 반영
  - OrderListItemActions 컴포넌트 업데이트
    - products 정보를 props로 받도록 수정
    - 재주문 및 주문 취소 기능 연결
    - 버튼 크기 반응형 적용
- 2025-01-XX: 주문 내역 페이지 모바일/태블릿 반응형 개선
  - 주문 목록 페이지(OrdersClient.tsx) 반응형 개선
    - 컨테이너 패딩: 모바일(px-4), 태블릿(px-6), 데스크톱(px-8~16)
    - 헤더 텍스트 크기: 모바일(text-2xl), 태블릿(text-3xl), 데스크톱(text-4xl~6xl)
    - 검색 아이콘 크기: 모바일(w-5 h-5), 태블릿(w-6 h-6), 데스크톱(w-8~10 h-8~10)
    - 검색창 패딩: 모바일(pl-10 py-3), 태블릿(pl-12 py-4), 데스크톱(pl-16~20 py-5~6)
    - 상태 필터 버튼: 모바일(px-4 py-2 text-xs), 태블릿(px-5 py-2.5 text-sm), 데스크톱(px-6~8 py-3~4 text-base~lg)
    - 주문 카드 패딩: 모바일(p-4), 태블릿(p-5~6), 데스크톱(p-8~12)
    - 텍스트 크기 및 간격 전체적으로 모바일/태블릿/데스크톱에 맞게 조정
  - 주문 상세 페이지([id]/page.tsx) 반응형 개선
    - 컨테이너 패딩: 모바일(px-4 py-4), 태블릿(px-5 py-5), 데스크톱(px-6~8 py-6~8)
    - 제목 크기: 모바일(text-lg), 태블릿(text-xl), 데스크톱(text-2xl~3xl)
    - 타임라인 아이콘 크기: 모바일(w-6 h-6), 태블릿(w-7 h-7), 데스크톱(w-8 h-8)
    - 배송 상태 배너: 모바일(p-3 text-sm), 태블릿(p-4 text-base), 데스크톱(p-4 text-base)
    - 카드 패딩 및 텍스트 크기 전체적으로 모바일/태블릿/데스크톱에 맞게 조정
- 2025-12-11: 재구매 버튼 기능 구현
  - 주문 상세 페이지의 재구매 버튼 클릭 시 해당 상품 상세 페이지로 이동 기능 추가
  - useRouter를 사용하여 `/retailer/products/{id}` 경로로 이동
  - 재구매 클릭 시 로그 출력 (상품 ID, 상품명)
  - 다크모드 대응 스타일 추가
- 2025-12-11: 주문 상세 페이지에서 리뷰 쓰기 버튼 제거
  - OrderProductItem 컴포넌트에서 리뷰 쓰기 버튼 삭제
  - 재구매 버튼만 유지
- 2025-12-11: 장바구니 페이지 다크모드 대응 개선
  - 장바구니 페이지 제목 텍스트에 다크모드일 때 흰색으로 표시되도록 `dark:text-white` 클래스 추가
  - 다크모드에서 가독성 향상
